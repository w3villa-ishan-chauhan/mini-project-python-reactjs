...
firebase_admin.initialize_app(
    cred,
    {   
        "storageBucket": "mini-project-profile-pic.appspot.com",
    },
)
...
async def handle_payment(
    amount: Amount, authorization: str = Header(...), db: Session = Depends(getDb)
):
    token = authorization.split(" ")[1]
    email = auth.decodeAccessToken(token)
    current_user = auth.getUser(db, email)
    ...
    if payment_intent["amount"] == 1000:
        current_user.subscription_type = "Gold"
    elif payment_intent["amount"] == 500:
        current_user.subscription_type = "Silver"
    db.commit()
    db.refresh(current_user)

    print("user_subs_type:", current_user.subscription_type)
